...
import com.mydemo.demo.common.Rest;
...
public class JWTFilter extends BasicHttpAuthenticationFilter {
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
        log.debug("come in");
        //判断请求头部是否有Authorization，有的话进行登录认证
        //请求头不存在token，则可能是登录操作或游客身份
    ...
    private void response401(ServletResponse response, String msg) {
        ...
        for (Errcode errcode : values) {
            if (errcode.getErrmsg().equals(msg)) {
                code = errcode.getErrcode();
                break;
            }
        }
        ...
        try (PrintWriter out = httpServletResponse.getWriter()) {
            String data = JSONObject.toJSONString(new Rest().rest(code, msg));
            out.append(data);
        } catch (IOException e) {
            log.error("直接返回Response信息出现IOException异常:{}", e.getMessage());
            throw new ServerException(-1, "直接返回Response信息出现IOException异常:" + e.getMessage());
        }
    }
    ...
    private boolean refreshToken(ServletRequest request, ServletResponse response) {
        ...
        if (redisUtil.hasKey(RedisKeys.PREFIX_SHIRO_REFRESH_TOKEN + username)) {
            // Redis中RefreshToken还存在，获取RefreshToken的时间戳
            String currentTimeMillisRedis = redisUtil.get(RedisKeys.PREFIX_SHIRO_REFRESH_TOKEN + username);
            // 获取当前AccessToken中的时间戳，与RefreshToken的时间戳对比，如果当前时间戳一致，进行AccessToken刷新
            if (JwtUtil.isRefreshExpired(token, currentTimeMillisRedis)) {
                // 获取当前最新时间戳
